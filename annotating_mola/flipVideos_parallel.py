...
def _process_one(in_path_str: str, out_dir_str: str):
    """
    Worker function to process a single input video.
    Kept logic identical to the original loop body.
    """
    ...
    cmd = ["ffmpeg", "-threads", "2", "-y", "-hide_banner", "-loglevel", "error", "-i", str(in_path)]
...
def main(videos_dir: str):
    print(f"Flipping videos in directory: {videos_dir}")
    ...
    # Default parallelism: one worker per CPU core (same as ProcessPoolExecutor default)
    with ProcessPoolExecutor(max_workers=6) as ex:
        futures = [ex.submit(_process_one, str(in_path), str(out_dir)) for in_path in mp4s]
        for f in as_completed(futures):
            # Re-raise unexpected worker crashes (keeps original per-file failure printing intact).
            f.result()

    print(f"Finished all videos, count = {len(mp4s)}")
