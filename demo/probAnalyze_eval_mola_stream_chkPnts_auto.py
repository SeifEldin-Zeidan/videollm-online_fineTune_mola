...
from .probAnalyze_inference_eval import LiveInfer
...
def main(liveinfer: LiveInfer, dataset_annos, pt_root_path, results_path, doFlip=True):
    ...
    for videoName, videoAnno in tqdm.tqdm(dataset_annos.items()):
        ...
        for i in range(num_video_frames):
            ...
            query, response, staySilentProb = liveinfer()
            print(f"staySilentProb: {staySilentProb}")
            end_time = time.time()
            ...
            if not query and not response:
                history['conversation'].append({'time': liveinfer.video_time, 'fps': fps, 'cost': timecosts[-1], 'staySilentProb': staySilentProb.item()})
...
if __name__ == '__main__': #run with 2 fps annotations
    ...
    parser.add_argument("--thresh", type=float, required=False, default=0.0)
    ...
    if script_args.thresh != 0.0:
        ValueError("You are not setting the thresh to 0.0, so you are not analyzing probs!")
    ...
    chk_root_path = None
    index_chk_path = None
    for i, m_arg in enumerate(model_args):
        if m_arg == "--resume_from_checkpoint":
            chk_root_path = Path(model_args[i+1])
            index_chk_path = i+1

    if not chk_root_path:
        print(f"Couldn't find arg --resume_from_checkpoint")
        

    results_root_path_base = results_root_path
    for dir in Path(chk_root_path).iterdir():
        if dir.is_dir():
            if "checkpoint" in dir.name:
                ...
                print(f"dir = {dir}")
                model_args[index_chk_path] = str(dir)

                results_root_path = os.path.join(results_root_path_base, f"train_{train_num}", f"chk_{chk_num}")
                ...
                try:
                    ...
                    print(f"Exception: {e}")
                    failure_file_path = os.path.join(results_root_path, f"{chk_num}_failure.txt")
                    with open(failure_file_path, 'w') as f:
                        f.write(f"Checkpoint {chk_num} failed!")
                        f.write(f"\n Exception:")
                        f.write(str(e))
