...
class LiveInfer:
    def __init__(self, ) -> None:
        ...
        self.model, self.tokenizer = build_model_and_tokenizer(is_training=False, set_vision_inside=False, **asdict(args))
        self.model.to('cuda')
    ...
    def _call_for_streaming(self, ):
        while self.frame_embeds_queue:
            ...
            staySilentProb = next_score[:,:,self.frame_token_interval_id]
            ...
            ## Never respond! comment this part
            # if self.last_ids != self.frame_token_interval_id: 
            #     return video_time, None, staySilentProb

        return None, None, staySilentProb
    ...
    def input_video_stream(self, video_time): #NOTE:  set_vision_inside=False, 
    ...
    def load_video(self, video_path):
        ...
        self.video_tensor = torch.load(video_path, weights_only=False).to("cuda") # T x N x 1024
    ...
    def __call__(self, ):
        while not self.frame_embeds_queue:
            continue
        video_time, query, staySilentProb = self._call_for_streaming()
        response = None
        if video_time is not None:
            query, response, staySilentProb = self._call_for_response(video_time, query)
        return query, response, staySilentProb