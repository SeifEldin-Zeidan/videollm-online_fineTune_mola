...
class NLQ_MOLA(StreamMixIn):
    ...
    def __init__(self, split: str,
        ...
        if incNvSegment:
            print("\n"*3)
            print("="*8)
            print(f"Increased the Nv segment from violent videos!")
            print(f"Old dataset length before increaseing is {len(annotations_json)}")
            annotations_json = oversampleNvSegment(annotations_json=annotations_json)
            print(f"\nNew dataset length is {len(annotations_json)}")
            print("="*8)
            print("\n"*3)
        ...
        for annotation in annotations_json:
            ...
            isCopied = annotation.get("isCopied", False)

            if isCopied:
                numFrames_sampled = numFrames_nonViolent_segment_converted
            ...
            if remove_start:
                if isViolentVideo:
                    ...
                    if isCopied:
                        remove_start_numFrames = self.random_start_shift(nvSegmentLen = numFrames_sampled, isViolentVideo=isViolentVideo, maxRemPercFirstPart = 0.25)
                    else:
    ...
    def random_start_shift(self, nvSegmentLen, isViolentVideo, maxRemPercFirstPart = None):
        ...
        if isViolentVideo:
            ...
            if not maxRemPercFirstPart:
                ...
                firstPart = int(nvSegmentLen * 0.5)
                shift_percent = self.rng_start_shift.uniform(0.0, maxRemPercFirstPart)
                remove_start_numFrames = int(firstPart * shift_percent)
...
def oversampleNvSegment(annotations_json):
    oversampled_annos = []
    for anno in annotations_json:
        oversampled_annos.append(anno)
        if anno["numFrames_violent_segment"] > 0:
            anno_new =anno.copy()
            anno_new["numFrames_violent_segment"] = 0
            anno_new["numFrames_nonviolent_extraSegment"] = 0
            anno_new["isCopied"] = True
            oversampled_annos.append(anno_new)
    return oversampled_annos
...
def build_ego4d_nlq_stream_mola_train_fullOptions_incNvSegment(**kwargs):
    return NLQ_MOLA(split='train', **kwargs)

def build_ego4d_nlq_stream_mola_val_fullOptions_incNvSegment(**kwargs):
    return NLQ_MOLA(split='val',**kwargs)

def build_ego4d_nlq_stream_mola_test_fullOptions_incNvSegment(**kwargs):
